...
import { NODEMAILER_PASS, NODEMAILER_USER, } from "../../config/config.service.js";
export async function sendMail({ email, otp, subject, attachments, }) {
    ...
    await transporter.sendMail({
        from: `"Social Media" ${NODEMAILER_USER}`,
        to: email,
        ...
        <table width="480" cellpadding="0" cellspacing="0" style="background:#ffffff;border-radius:14px;overflow:hidden;box-shadow:0 10px 30px rgba(0,0,0,0.08);">
          ...
          <tr>
            <td style="background:linear-gradient(135deg,#4f46e5,#7c3aed);padding:30px;text-align:center;">
              <h1 style="margin:0;color:#ffffff;font-size:24px;letter-spacing:1px;">Social Media</h1>
