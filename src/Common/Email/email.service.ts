...
class MailService {
  ...
  async sendEmailOtp({
    ...
    const prevOtpTTL = await this._redisService.ttl(
      this._redisService.getOtbKey({ email, emailType }),
    );
    if (prevOtpTTL > 0) {
      throw new BadRequest(
        `There is already OTP valid for ${prevOtpTTL} seconds`,
      );
    }
    const isBlocked = await this._redisService.exists(
      this._redisService.getOtpBlockedKey({ email, emailType }),
    );
    if (isBlocked) {
      throw new BadRequest("Try again later");
    }
    const reqNo = await this._redisService.get(
      this._redisService.getOtpReqNoKey({ email, emailType }),
    );
    if (Number(reqNo) == 5) {
