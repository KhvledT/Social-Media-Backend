...
export function authentication(tokenTypeParm = TokenTypeEnum.access) {
  return async (req: Request, res: Response, next: NextFunction) => {
    ...
    const decodedToken = TokenServices.decodeToken(token) as JwtPayload;
    ...
    const { accessSignature, refreshSignature } = TokenServices.getSignature(
    ...
    const verifiedToken = TokenServices.verifyToken({
      token: token,
      signature:
        tokenTypeParm == TokenTypeEnum.access
          ? accessSignature
          : refreshSignature,
    }) as JwtPayload;
